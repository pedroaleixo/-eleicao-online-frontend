...
@Component({
  ...
  styleUrls: ['./eleitor-list.component.css']
})
export class EleitorListComponent implements OnInit {
  ...
  filtrar(){
    ...
    this.eleitorService.listarEleitoresPorFiltro(this.filtro, this.pageIndex, this.pageSize)
    .subscribe(page => {
  ...
  buscarEleitores(event:PageEvent){
    this.userService.getUser().subscribe(user => {
      if(user?.pessoa) {
        this.eleitorService.listarEleitoresPorFiltro(this.filtro, event.pageIndex,
          event.pageSize)
        .subscribe(page => {
          this.dataSource.data = page.content;
  ...
  redirecionarParaEdicao(id:number){
    this.router.navigate([`/eleitor/form/${id}`]);
  }
  ...
  confirmarRemocao(id:number){
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result){
        this.eleitorService.remover(id).subscribe(resp => {
          window.scroll(0,0);
          this.snackbarService.success('Eleitor removido com sucesso');
          this.limpar();
        });
      }
    });
